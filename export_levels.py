...
def export_compressed_to_s_file(level_name, compressed, output_path):
    ...
    with open(output_path, 'w') as file:
        ...
        file.write(".align 2\n")
        file.write(f".global {level_name}_level_data\n")
        file.write(f".hidden {level_name}_level_data\n")
        file.write(f"{level_name}_level_data:\n")
        for value, count in compressed:
            file.write(f"    .hword {value}\n    .hword {count}    @ Value {value} repeats {count} times\n")
            
def export_header_file(level_name, level_array, compressed, output_path):
    ...
    with open(output_path, 'w') as file:
        ...
        file.write(f"#define {level_name.upper()}_TOTAL_BLOCKS {len(level_array) * len(level_array[0])}\n")
        file.write(f"#define {level_name.upper()}_RLE_DATA_SIZE {rle_size}\n")

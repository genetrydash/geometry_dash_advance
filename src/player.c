...
void do_ball_gravity() {
    s8 sign = (curr_player.gravity_dir == GRAVITY_UP) ? -1 : 1;
    curr_player.player_y_speed += (curr_player.gravity / 4) * sign;
}
    ...
    if (curr_player.relative_player_y > -48 && curr_player.relative_player_y < SCREEN_HEIGHT + 48) {
        switch (curr_player.gamemode) {
            case GAMEMODE_CUBE:
                ...
                oam_metaspr(curr_player.relative_player_x - x_offset, curr_player.relative_player_y - y_offset, player_sprite, FALSE, FALSE, 0, -1, priority, FALSE);
                break;
            case GAMEMODE_SHIP:
                ...
                oam_metaspr(curr_player.relative_player_x - 8, curr_player.relative_player_y - y_offset, player_sprite, FALSE, FALSE, 0, -1, priority, FALSE);
                break;
            case GAMEMODE_BALL:
